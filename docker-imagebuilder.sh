...
for TARGET in $TARGETS ; do
    ...
    for BRANCH in $BRANCHES; do
        ...
        docker build -t $DOCKER_USER/$DOCKER_IMAGE:$TARGET-$BRANCH -f Dockerfile.imagebuilder .

        rm -rf ./imagebuilder

        if [ "$BRANCH" == "master" ]; then
            docker tag $DOCKER_USER/$DOCKER_IMAGE:$TARGET-$BRANCH $DOCKER_USER/$DOCKER_IMAGE:$TARGET
            docker push $DOCKER_USER/$DOCKER_IMAGE:$TARGET
        else
            docker push $DOCKER_USER/$DOCKER_IMAGE:$TARGET-$BRANCH
        fi
    done
done
